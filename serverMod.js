...
module.exports.initMod = function (io, gameState, DATA) {
	...
	global.volunteerTalk = [
	"Welcome to the Zone!",
	"Don't forget your lanyard!",
	"Enjoy Freeplay!"
	];
	
	global.currentVideo = "ReJc4szeQM0";
	
	global.nextTrackTalk = [
	"Next track!",
	"Rewind!",
	"Another one!",
	"They don't stop!",
    ...
    var npc = new NPC(
        {
            id: "volunteer",
            nickName: "Volunteer Bot",
            room: "freeplayOutside",
            x: 19,
            y: 86,
            avatar: 1,
            colors: [2, 2, 1, 5],
    ...
    npc.behavior = setTimeout(function ramble() {
        ...
        if (dice < 40) {
            npc.talk(global.volunteerTalk[Math.floor(random(0, global.volunteerTalk.length))]);

            npc.behavior = setTimeout(ramble, random(7000, 9000));
        }
        else if (dice < 60) {
            npc.move(random(4, 46) * 2, random(82, 94) * 2);
...
module.exports.onGetLanyard = function (pId)
{
	// give player lanyard
	var player = gameState.players[pId];
	if(player.hasLanyard == null || player.hasLanyard == false)
	{
		player.hasLanyard = true;
		console.log(player.nickName + " picked up their lanyard!");
		// tell them they got their lanyard
		io.sockets.sockets[pId].emit('godMessage', "You pick up your festival lanyard.");
	}
	else
	{
		io.sockets.sockets[pId].emit('godMessage', "You already have a lanyard!");
	}
}

module.exports.onMoreZone = function (pId)
{
	if(exhibitionOn == true)
	{
		this.transferPlayer(pId, gameState.players[pId].room, "market", 99 * 2, 92 * 2);
	}
	else
	{
		this.transferPlayer(pId, gameState.players[pId].room, "backyard", 12 * 2, 86 * 2);
	}
}

module.exports.onEnterZone = function (pId)
{
	var player = gameState.players[pId];
	var canEnter = false;
	if(gameState.players[pId].room == "backyard")
	{
		canEnter = true;
	}
	...
	else if(player.hasLanyard != null && player.hasLanyard == true)
	{
		canEnter = true;
	}
	...
	if(canEnter == false)
	{
		io.sockets.sockets[pId].emit('godMessage', "You need a festival lanyard to enter!");
	}
	else
	{
		if(exhibitionOn == true)
		...
		{
			this.transferPlayer(pId, gameState.players[pId].room, "screeningRoom", 96 * 2, 91 * 2);
		}
	}
}

module.exports.onEnterScreeningRoom = function (pId)
{
	var player = gameState.players[pId];
	if(player.admin = 0 || player.hasLanyard == null || player.hasLanyard == false)
	{
		io.sockets.sockets[pId].emit('godMessage', "You need a festival lanyard to enter!");
	}
	else
	{
		this.transferPlayer(pId, "freeplayDay", "screeningRoom", 96 * 2, 91 * 2);
	}
}

//wouldn't it be funny if cetain rooms modified your messages?
module.exports.screeningRoomTalkFilter = function (player, message)
{
	console.log("MOD: " + player.nickName + " said " + message);
	var cmd = message.split(" ");
	if(cmd.length == 1 && cmd[0] == "reload")
	{
		io.to("screeningRoom").emit("reloadVideo", global.currentVideo);
		return "Adjusting video!";
	}
	else if(cmd.length == 2 && cmd[0] == "video")
	{
		global.currentVideo = cmd[1];
		io.to("screeningRoom").emit("changeVideo", global.currentVideo);
		return global.nextTrackTalk[Math.floor(random(0, global.nextTrackTalk.length))];
	}
	//make sure it returns a message
	return message;
}

module.exports.screeningRoomJoin = function (playerObject, roomId)
{
	io.to(playerObject.id).emit('changeVideo', global.currentVideo);
}

module.exports.backyardJoin = function (playerObject, roomId)
{
	if(exhibitionOn == false)
	{
		io.to(playerObject.id).emit('hideGame');
	}
}

module.exports.exhibitionToggle = function (state)
{
	if(state == "off")
	{
		exhibitionOn = false;
