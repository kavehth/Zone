...
function initMod(playerId, roomId) {
	...
	socket.on('hideGame', function ()
	{
		removeThing("middeOfTheHill","backyard");
	});
    }
...
function firstFloorEnter(playerId, roomId) {
	...
	function screeningRoomEnter(playerId, roomId)
	{
	...
	function screeningRoomExit(playerId)
	{
		if(playerId == me.id)
		{
	...
	function backyardEnter(playerId, roomId)
	{
		if(playerId == me.id)
		{
			e = document.getElementById("video-container");
	...
	function marketEnter(playerId, roomId)
	{
		if(playerId == me.id)
		{
			e = document.getElementById("video-container");
